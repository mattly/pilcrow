              ...
              content (gen/such-that
                       ...
                       (gen/fmap
                        ...
                        (apply
                         ...
                         (filter
                          identity
                          [(gen/vector gen-paragraph 0 10)
                           (when (and subsec (> 6 level))
                             (gen/vector (gen-section-with-content (inc level)) 0 msize))])))
                       100)]
  ...
  (gen/fmap
   ...
   (gen/fmap
    ...
    (gen/let
      ...
      [head body #_(if head (update body 0 (fn [[tag & c]] [tag])) body)]))))
...
;; (deftest paragraphs
;;   (checking "separates paragraphs" {:num-tests 50}
;;     [paras (gen/vector gen-paragraph 1 30)]
;;     (println "paras")
;;     (is (= paras
;;            (pilcrow/parse-para (str/split-lines (render paras))))
;;         (render paras))))
